...
load_detected() {
    # ── 缓存有效性检查：协议切换（X11↔Wayland）后强制重新检测 ────────────────
    if [ -f "$DETECTED_FILE" ]; then
        local cached_protocol current_protocol
        cached_protocol=$(grep "^DISPLAY_PROTOCOL=" "$DETECTED_FILE" 2>/dev/null \
                          | cut -d= -f2 | tr -d '[:space:]')
        # 读取当前实际协议
        if   [ "${XDG_SESSION_TYPE:-}" = "wayland" ] || [ -n "${WAYLAND_DISPLAY:-}" ]; then
            current_protocol="wayland"
        elif [ "${XDG_SESSION_TYPE:-}" = "x11" ]    || [ -n "${DISPLAY:-}" ]; then
            current_protocol="x11"
        else
            current_protocol="unknown"
        fi
        # 协议不一致且均非 unknown → 缓存已过期，删除后重新检测
        if [ -n "$cached_protocol" ] \
        && [ "$cached_protocol" != "unknown" ] \
        && [ "$current_protocol" != "unknown" ] \
        && [ "$cached_protocol" != "$current_protocol" ]; then
            print_info "检测到显示协议已变更（${cached_protocol} → ${current_protocol}），重新扫描硬件..."
            rm -f "$DETECTED_FILE"
        fi
    fi
...
cmd_detect() {
    ...
    # 给出安装建议（根据发行版族显示对应的包管理器命令）
    local _install_hint=""
    if   command -v apt     >/dev/null 2>&1; then _install_hint="sudo apt install"
    elif command -v dnf     >/dev/null 2>&1; then _install_hint="sudo dnf install"
    elif command -v pacman  >/dev/null 2>&1; then _install_hint="sudo pacman -S"
    elif command -v zypper  >/dev/null 2>&1; then _install_hint="sudo zypper install"
    else                                          _install_hint="<你的包管理器> install"
    fi

    if ! $TOOL_DDCUTIL; then
        print_info "建议安装 ddcutil 以支援外接显示器硬件控制："
        echo -e "  ${CYAN}${_install_hint} ddcutil${NC}"
    fi
    if ! $TOOL_BRIGHTNESSCTL; then
        print_info "建议安装 brightnessctl 以支援内置屏幕控制："
        echo -e "  ${CYAN}${_install_hint} brightnessctl${NC}"
    fi
