...
class BrightnessWindow(tk.Toplevel):
    ...
    def _build_monitor_card(self, mon):
        ...
        # 窗口建立完成后，背景线程异步读取实际亮度，不阻塞 UI
        # 读到后切回主线程更新滑杆（仅在首次建立时更新，避免干扰用户拖动）
        def _async_read_brightness(dev=device, bvar=bright_var, blabel=bright_label):
            actual = self._read_actual_brightness(dev)
            if actual is not None:
                # 用 after(0) 切回主线程安全更新 tkinter 变量
                self.after(0, lambda v=actual: (
                    bvar.set(v),
                    blabel.config(text=f"{v}%")
                ))
        threading.Thread(target=_async_read_brightness, daemon=True).start()
    ...
    def _redetect(self):
        """重新检测显示器（全程在背景线程执行 IO，主线程零等待）"""
        def do():
            run_bash(["detect"])
            # IO 全部在背景线程完成
            monitors = load_detected()
            settings = load_settings()
            # 读完再切回主线程重建窗口
            self.after(0, lambda m=monitors, s=settings: self._reload_monitors(m, s))
        threading.Thread(target=do, daemon=True).start()

    def _reload_monitors(self, monitors, settings):
        """在主线程重建窗口（此时数据已就绪，无 IO）"""
        parent = self.parent
        self.close()
        win = BrightnessWindow(parent, monitors, settings)
        win.grab_set()
...
class TrayApp:
    """
    屏幕亮度控制托盘应用。
    架构：tkinter 始终在主线程运行（mainloop），浮动按钮作为托盘替代方案。
    背景线程负责硬件检测与亮度恢复，检测结果缓存到 detected.conf。
    """
    ...
    def _init_background(self):
        """背景线程：检测硬件（必要时）并恢复上次亮度设定"""
        # detected.conf 缺失，或协议已切换（由 screen-brightness.sh 负责判断）→ 重新检测
        if not os.path.exists(DETECTED_FILE):
            try:
            ...
            # 检查缓存中的协议是否与当前会话一致；不一致则触发重新检测
            try:
                cached_proto = ""
                with open(DETECTED_FILE) as f:
                    for line in f:
                        if line.startswith("DISPLAY_PROTOCOL="):
                            cached_proto = line.strip().split("=", 1)[1]
                            break
                current_proto = os.environ.get("XDG_SESSION_TYPE", "")
                if not current_proto:
                    current_proto = "wayland" if os.environ.get("WAYLAND_DISPLAY") else                                     "x11"     if os.environ.get("DISPLAY")         else ""
                if (cached_proto and current_proto
                        and cached_proto != "unknown"
                        and current_proto != "unknown"
                        and cached_proto != current_proto):
                    # 协议不一致：删除旧缓存，触发 screen-brightness.sh 重新检测
        ...
        # 在背景线程里读好数据，然后用 root.after 安全交还给主线程
        # 绝对不能在背景线程里直接写 self.monitors / self.settings（线程不安全）
        monitors = load_detected()
        settings = load_settings()
        self.root.after(0, lambda m=monitors, s=settings: self._on_bg_ready(m, s))

    def _on_bg_ready(self, monitors, settings):
        """背景线程完成后，在主线程安全更新缓存（zero-latency）"""
        self.monitors = monitors
        self.settings = settings
    ...
    def _toggle_popup(self):
        if self.popup and self.popup.winfo_exists():
            self.popup.close()
            self.popup = None
            # 让 tkinter 把关闭事件全部处理完，
            # 再开新窗口前给 Wayland compositor 一帧时间释放旧缓冲区
            self.root.update_idletasks()
            return   # 单次点击只关不开，避免立刻重建造成 Wayland 竞争
        else:
            # 永远立即弹窗，不等任何 IO。
            # self.monitors 由 _on_bg_ready 在背景线程完成后填入，
            # 若还是空列表（HDD 首次启动尚未读完），窗口显示 loading 提示，
            # 背景线程完成后会自动刷新窗口内容。
            self.popup = BrightnessWindow(self.root, self.monitors, self.settings)
            # 若此时背景还未就绪，注册一个轮询：就绪后自动刷新已打开的窗口
            if not self.monitors:
                self._wait_and_refresh_popup()

    def _wait_and_refresh_popup(self, _retries=0):
        """轮询背景线程是否就绪，就绪后刷新已打开的窗口（不重建，无闪烁）"""
        MAX_RETRIES = 60  # 最多等 6 秒（每次 100ms）
        if self.monitors:
            # 数据已就绪，刷新窗口
            if self.popup and self.popup.winfo_exists():
                self.popup.close()
                self.popup = BrightnessWindow(self.root, self.monitors, self.settings)
        elif _retries < MAX_RETRIES:
            self.root.after(100, lambda: self._wait_and_refresh_popup(_retries + 1))
        # 超时则放弃，用户可点 ⟳ 手动刷新
    ...
    def _build_fallback_button(self):
        ...
        def _quit_app():
            """彻底退出：destroy 比 quit 更完整地断开 Wayland/X11 连接"""
            try:
                self.root.destroy()
            except Exception:
                pass
        ...
        quit_btn.bind("<Button-1>", lambda e: _quit_app())
