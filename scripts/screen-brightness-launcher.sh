...
# 安全停止托盘：等待进程真正退出后才返回
# Wayland 下 compositor 需要额外时间释放窗口缓冲区，
# 若 pkill 后立刻启动新进程会造成资源竞争（卡顿/图标不出现）
stop_tray_and_wait() {
    if ! pgrep -f "screen-brightness-tray.py" >/dev/null 2>&1; then
        return 0
    fi

    pkill -TERM -f "screen-brightness-tray.py" 2>/dev/null || true

    local _wait=0
    while pgrep -f "screen-brightness-tray.py" >/dev/null 2>&1; do
        sleep 0.3
        _wait=$(( _wait + 1 ))
        # 超过 3 秒还没退出，升级为 SIGKILL
        [ "$_wait" -ge 10 ] && pkill -KILL -f "screen-brightness-tray.py" 2>/dev/null || true
        # 最多再等 1.5 秒让 SIGKILL 生效
        [ "$_wait" -ge 15 ] && break
    done

    # 额外等待：给 Wayland compositor 时间释放图形缓冲区
    # X11 不需要，但多等 0.5s 无害
    sleep 0.5
}
