...
set +e   # 不能用 set -e：_pkg_installed() 靠非零返回码表示"未安装"，set -e 会误判为错误并退出
...
PKG_MAP=(
    ...
    # cups-browsed: Ubuntu/Debian 22.04+ 已拆为独立包；Arch/SUSE 合并在 cups-filters 里，填 - 跳过
    "cups-browsed|cups-browsed|cups-browsed|-|-"
...
do_launch_gui() {
    ...
    # 等待 1.5 秒，检测 GUI 是否立即崩溃（如 python3 找不到 tkinter 等）
    sleep 1.5
    if ! kill -0 "$gui_pid" 2>/dev/null; then
        print_error "图形界面启动后立即退出，请检查 python3-tk 是否已安装。"
        print_warning "可选菜单 [2] 重新检测/修复驱动环境。"
    else
        print_success "图形界面已成功启动（PID: ${gui_pid}）"
    fi
}
