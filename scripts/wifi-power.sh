...
latency_verdict() {
    ...
    # 平均延迟正常，但峰值过高 → 有间歇性抖动
    elif (( $(echo "$max > 200" | bc -l 2>/dev/null || echo 0) )); then echo "有严重抖动"
    elif (( $(echo "$max > 50"  | bc -l 2>/dev/null || echo 0) )); then echo "有轻微抖动"
    else echo "稳定"
    fi
}
...
run_stability_test() {
    ...
    (( $(echo "$max_ms > 200" | bc -l 2>/dev/null || echo 0) )) && max_color=$RED   || true
    (( $(echo "$max_ms > 50"  | bc -l 2>/dev/null || echo 0) )) && [ "$max_color" = "$GREEN" ] && max_color=$YELLOW || true
