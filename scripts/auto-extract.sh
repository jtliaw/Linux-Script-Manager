#!/bin/bash
# Auto Extract
# DESCRIPTION: 智能批量解压工具 - 支持所有压缩格式的递归解压（支持多个发行版）
# REQUIRES_SUDO: false

# ============================================
# 智能批量解压脚本 v2.3
# 支持自动检测系统、安装依赖、无限递归解压
# 智能防止文件冲突、实时进度显示
# ============================================
...
install_dependencies() {
    ...
    if [ "$PKG_MANAGER" = "none" ]; then
        ...
        echo "  - unzip"
        echo "  - unrar 或 unar"
        echo "  - p7zip-full 或 7zip"
        echo "  - tar"
        echo "  - gzip"
        echo "  - bzip2"
        echo ""
        read -p "$(echo -e "${PURPLE}[INPUT]${NC} 是否继续使用已安装的工具? [y/N]: ")" continue_anyway
    ...
    case "$PKG_MANAGER" in
        ...
        dnf)
            for pkg in "${missing_packages[@]}"; do
                sudo dnf install -y $pkg 2>/dev/null || print_warning "无法安装 $pkg"
            done
            ;;
        yum)
            for pkg in "${missing_packages[@]}"; do
                sudo yum install -y $pkg 2>/dev/null || print_warning "无法安装 $pkg"
            done
            ;;
        pacman)
            for pkg in "${missing_packages[@]}"; do
        ...
        pkg)
            for pkg in "${missing_packages[@]}"; do
                sudo pkg install -y $pkg 2>/dev/null || print_warning "无法安装 $pkg"
            done
            ;;
        emerge)
            for pkg in "${missing_packages[@]}"; do
                sudo emerge --ask=n $pkg 2>/dev/null || print_warning "无法安装 $pkg"
            done
            ;;
        xbps-install)
            for pkg in "${missing_packages[@]}"; do
                sudo xbps-install -y $pkg 2>/dev/null || print_warning "无法安装 $pkg"
            done
            ;;
        nix-env)
            for pkg in "${missing_packages[@]}"; do
                nix-env -iA nixpkgs.$pkg 2>/dev/null || print_warning "无法安装 $pkg"
            done
            ;;
        brew)
            for pkg in "${missing_packages[@]}"; do
                brew install $pkg 2>/dev/null || print_warning "无法安装 $pkg"
            done
            ;;
    esac
...
check_archive_structure() {
    ...
    case "$extension" in
        zip)
            ...
            local contents=$(unzip -l "$file" 2>/dev/null | awk 'NR>3 {print $4}' | grep -v '^$' | head -20)
            ;;
        rar)
            ...
            local contents=$(unrar lb "$file" 2>/dev/null | head -20)
            ;;
        7z)
            echo "multi_root"
            return
...
extract_file() {
    ...
    local structure=$(check_archive_structure "$file")
    
    local target_dir
    local need_wrapper_dir=false
    
    if [ "$structure" = "single_root" ]; then
        target_dir="$file_dir"
        print_step "检测到单根目录结构，直接解压"
    else
        target_dir="$file_dir/$basename"
        need_wrapper_dir=true
        mkdir -p "$target_dir"
        print_step "检测到多根结构，创建隔离目录: $basename/"
    fi
    ...
    case "$extension" in
        ...
        rar)
            ...
            if [ -n "$password" ]; then
                unrar x -p"$password" -o+ -inul "$file" "$target_dir/" 2>/dev/null && extract_success=true
            else
                unrar x -o+ -inul "$file" "$target_dir/" 2>/dev/null && extract_success=true
            fi
            ;;
        7z)
            ...
            if [ -n "$password" ]; then
                timeout 60s 7z x -p"$password" -o"$target_dir" -y "$file" >/dev/null 2>&1 && extract_success=true
            else
                timeout 60s 7z x -o"$target_dir" -y "$file" >/dev/null 2>&1 && extract_success=true
            fi
            
            if [ $? -eq 124 ]; then
                print_error "  └─ 解压超时（60秒）"
                return 1
            fi
            ;;
        tar)
    ...
    if [ "$extract_success" = true ]; then
        ...
        if [ "$need_wrapper_dir" = true ] && [ -d "$target_dir" ]; then
...
main() {
    ...
    if [ -n "$1" ]; then
        ...
        read -p "$(echo -e "${PURPLE}[INPUT]${NC} 请输入要解压的目录路径 (默认当前目录): ")" TARGET_DIR
    ...
    echo -e "${YELLOW}━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━${NC}"
    echo ""
    read -p "$(echo -e "${PURPLE}[INPUT]${NC} 请输入解压密码 (如无密码直接回车): ")" PASSWORD
    ...
    read -p "$(echo -e "${PURPLE}[INPUT]${NC} 是否开始解压? [y/N]: ")" confirm
