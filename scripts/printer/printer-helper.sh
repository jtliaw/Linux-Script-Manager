...
# ─── 🛠️ 修复图形界面调用时找不到 lpinfo/lpadmin 等命令的问题 ───
# 图形程序(tkinter)间接启动本脚本时，继承的 PATH 往往不包含 /usr/sbin、/sbin，
# 而 lpinfo / lpadmin 等 CUPS 命令通常装在 /usr/sbin 下，导致脚本误判"未安装驱动"。
# 这里强制把常见的系统命令目录加入 PATH，确保脚本无论被谁调用都能找到这些命令。
