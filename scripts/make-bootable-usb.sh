...
write_iso_with_dd() {
    ...
    # ── 进度条渲染函数（纯 bash，单行原地刷新，无额外依赖）──────────────────
    # 效果示例（自适应终端宽度）：
    #  ██████████████████░░░░░░░░  72%  1.3 GiB / 1.8 GiB  ⚡ 14.2 MiB/s  ETA 38s
    _render_progress() {
        local written=$1        # 已写字节
        local total=$2          # 总字节（0 = 未知）
        local elapsed=$3        # 已耗秒数
        local prev_written=$4   # 上一秒已写字节（用于算速）

        # ── 速度（bytes/s → MiB/s，纯整数运算）─────────────────────────────
        local delta=$(( written - prev_written ))
        local speed_mib_x10=$(( delta * 10 / 1048576 ))   # 扩大10倍保留一位小数
        local speed_int=$(( speed_mib_x10 / 10 ))
        local speed_dec=$(( speed_mib_x10 % 10 ))
        local speed_str="${speed_int}.${speed_dec} MiB/s"

        # ── ETA ──────────────────────────────────────────────────────────────
        local eta_str="--"
        if [ "$total" -gt 0 ] && [ "$delta" -gt 0 ]; then
            local remaining=$(( total - written ))
            local eta_sec=$(( remaining / delta ))
        ...
        # ── 人类可读大小（GiB，纯整数）──────────────────────────────────────
        local w_gib_x10=$(( written * 10 / 1073741824 ))
        local w_str="${w_gib_x10%?}.${w_gib_x10: -1} GiB"
        local t_str="? GiB"
        if [ "$total" -gt 0 ]; then
            local t_gib_x10=$(( total * 10 / 1073741824 ))
            t_str="${t_gib_x10%?}.${t_gib_x10: -1} GiB"
        fi

        # ── 进度条宽度自适应终端 ─────────────────────────────────────────────
        # 固定部分：" NN%  X.X GiB / X.X GiB  ⚡ XX.X MiB/s  ETA XXXs " ≈ 52字符
        local term_w; term_w=$(tput cols 2>/dev/null || echo 80)
        local bar_w=$(( term_w - 54 ))
        [ "$bar_w" -lt 8 ] && bar_w=8

        local filled=$(( pct * bar_w / 100 ))
        local empty=$(( bar_w - filled ))

        # ── 用 printf 拼接 bar（不用 awk，不用循环累加字符串）───────────────
        local bar_filled="" bar_empty=""
        [ "$filled" -gt 0 ] && bar_filled=$(printf '█%.0s' $(seq 1 "$filled"))
        [ "$empty"  -gt 0 ] && bar_empty=$( printf '░%.0s' $(seq 1 "$empty"))

        # ── 单行原地刷新：先清空当前行再打印，彻底避免残留字符 ───────────────
        printf "\r\033[2K${GREEN}%s${CYAN}%s${NC} ${WHITE}%3d%%${NC}  %s / %s  ${YELLOW}⚡ %s${NC}  ETA ${CYAN}%s${NC}" \
    ...
    _watch_dd_progress() {
        ...
        while kill -0 "$dd_pid" 2>/dev/null; do
            ...
            # 从 /proc/<pid>/io 读已写字节（内核直接提供，无需解析 dd 输出）
            local written=0
            if [ -f "/proc/$dd_pid/io" ]; then
                written=$(grep "^write_bytes:" "/proc/$dd_pid/io" 2>/dev/null | cut -d' ' -f2 || echo 0)
            fi

            _render_progress "$written" "$total" "$elapsed" "$prev_written"
            prev_written=$written
        done

        # dd 结束后打印 100% 并换行
        _render_progress "$total" "$total" "0" "0"
        echo ""
    }
    ...
    if command -v pv &> /dev/null; then
        print_info "使用 pv 显示进度..."
        echo ""
        if pv -p -t -e -r -b "$iso_file" | dd of="$device" bs=4M 2>/dev/null; then
            result=0
        fi

    # 方法2: dd status=progress（GNU coreutils 8.24+ 自带）
    elif dd if=/dev/null of=/dev/null bs=1 status=progress 2>&1 | grep -q "bytes"; then
        print_info "使用 dd 内置进度..."
        echo ""
        if dd if="$iso_file" of="$device" bs=4M status=progress 2>&1; then
            result=0
        fi

    # 方法3: 纯 bash 进度条（无任何额外依赖）
    else
        print_info "使用内建进度条..."
        echo ""
        # dd 后台运行
        dd if="$iso_file" of="$device" bs=4M 2>/dev/null &
        local dd_pid=$!
        # 进度监控（前台阻塞直到 dd 结束）
        _watch_dd_progress "$dd_pid" "$iso_size" "$start_time"
        # 取 dd 退出码
        wait "$dd_pid" && result=0 || result=$?
    fi
