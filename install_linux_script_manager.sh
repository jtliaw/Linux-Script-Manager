...
setup_package_manager() {
    case $DISTRO in
        ubuntu|debian|linuxmint|pop|zorin|peppermint)
            PKG_MANAGER="apt"
...
EOF
    ...
    # 自动检测桌面目录（兼容中文"桌面"及英文"Desktop"）
    local desktop_dir
    if command -v xdg-user-dir &> /dev/null; then
        desktop_dir="$(xdg-user-dir DESKTOP)"
    fi
    # 若 xdg-user-dir 不可用或返回 $HOME 本身，回退到常见名称
...
echo -e "  2. 桌面快捷方式: ${RED}$(xdg-user-dir DESKTOP 2>/dev/null || echo \"$HOME/Desktop\")/$DESKTOP_FILE${NC}"
...
# 删除桌面快捷方式
# 自动检测桌面目录
_DESKTOP_DIR="$(xdg-user-dir DESKTOP 2>/dev/null || true)"
[[ -z "$_DESKTOP_DIR" || "$_DESKTOP_DIR" == "$HOME" ]] && {
    [[ -d "$HOME/桌面" ]] && _DESKTOP_DIR="$HOME/桌面" || _DESKTOP_DIR="$HOME/Desktop"
}
if [[ -f "$_DESKTOP_DIR/$DESKTOP_FILE" ]]; then
    rm -f "$_DESKTOP_DIR/$DESKTOP_FILE"
...
if [[ "$UNINSTALL_MODE" == "standard" ]]; then
    ...
    # 删除用户设定目录
    for config_dir in \
        "$HOME/.config/linux-script-manager" \
        "$HOME/.config/screen-brightness"; do
        if [[ -d "$config_dir" ]]; then
            rm -rf "$config_dir"
            echo -e "${GREEN}✓ 已删除设定目录: $config_dir${NC}"
        fi
    done
...
show_completion_info() {
    ...
    echo -e "  桌面快捷方式: ${BLUE}$(xdg-user-dir DESKTOP 2>/dev/null || echo \"\$HOME/Desktop\")/$DESKTOP_FILE${NC}"
...
main() {
    ...
    echo -e "${CYAN}  支持: Ubuntu/Debian/Zorin/Peppermint, CentOS, Fedora, Arch, openSUSE${NC}"
