...
'''
    ...
    def _run_script_thread(self, script):
        ...
        try:
            ...
            if script['requires_sudo']:
                cmd = f'sh -c \'sudo "{script_path}"; echo "{press_enter}"; read\''
                subprocess.Popen([terminal, '-e'] + [cmd])
            else:
                cmd = f'sh -c \'bash "{script_path}"; echo "{press_enter}"; read\''
                subprocess.Popen([terminal, '-e'] + [cmd])
...
def main():
    ...
    try:
        ...
        print("On Ubuntu/Debian: sudo apt install python3-tk python3-pil")
